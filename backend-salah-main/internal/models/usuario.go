...
type Usuario struct {
	...
	Usuario            string `gorm:"column:usuario;not null;uniqueIndex" json:"usuario"`
	Contra             string `gorm:"column:contra;size:255;not null" json:"-"`
	Estado             bool   `gorm:"column:estado" json:"estado"`
	IDRol              uint   `gorm:"column:id_rol" json:"id_rol"`
	SessionVersion     uint   `gorm:"column:session_version;not null;default:1" json:"-"`
