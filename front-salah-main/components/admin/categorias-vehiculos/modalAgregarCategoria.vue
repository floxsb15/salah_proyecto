<template>
  <Dialog v-model:visible="visible" modal header="Agregar Nueva Categoría de Vehículo" :style="{ width: '25rem' }">
    <Form 
...
async function onFormSubmit({ valid } : any ) {
  if( valid ){
    ...
    const categoriaExistente = categorias.value.find(c => c.nombre?.toLowerCase() === nombreCategoria)
    ...
    try{
      await $fetch(server.HOST + '/api/v1/categorias-vehiculos', {
        method: 'POST',
        headers: {
