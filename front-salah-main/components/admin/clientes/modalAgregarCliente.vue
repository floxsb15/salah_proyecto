<template>
  ...
  >
    <div class="salah-client-modal">
      ...
      <Form v-slot="$form" :resolver="resolver" :initialValues="initialValues" @submit="onFormSubmit" class="client-form">
        ...
        <footer class="modal-actions">
          <Button type="submit" label="Registrar cliente" class="salah-submit" />
        </footer>
      </Form>
    </div>
  </Dialog>
...
async function onFormSubmit({ valid } : any ) {
  if (valid) {
    ...
    const ciDuplicado = clientes.value.find(c => normalizarCi(c.ci) === ci)
    ...
    try {
      await $fetch(server.HOST + '/api/v1/clientes', {
        method: 'POST',
        body: initialValues
      })
      emit('update')
...
.salah-input { width: 100%; }
:deep(.salah-input) { min-height: 50px; padding-left: 44px; border: 1px solid #d8d8d8; border-radius: 12px; background: #ffffff; color: #0d0d0d; box-shadow: none; transition: border-color .2s ease, box-shadow .2s ease; }
:deep(.salah-input:enabled:focus) { border-color: #ffd700; box-shadow: 0 0 0 4px rgba(255,215,0,0.18); }
