<template>
  <Dialog v-model:visible="visible" modal header="Modificar Segmento" :style="{ width: '25rem' }">
    <Form
      v-slot="$form"
      :resolver="resolver"
...
onMounted(async () => {
  try {
    const [resValue, resCategorias, resSegmentos]: any[] = await Promise.all([
      $fetch(server.HOST + '/api/v1/segmentos-vehiculos/' + props.id, { method: 'GET' }),
      $fetch(server.HOST + '/api/v1/categorias-vehiculos', { method: 'GET' }),
      $fetch(server.HOST + '/api/v1/segmentos-vehiculos', { method: 'GET' })
    ])

    initialValues.id_categoria = Number(resValue.id_categoria || 0)
    initialValues.nombre = resValue.nombre || ''
    initialValues.descripcion = resValue.descripcion || ''
    initialValues.estado = resValue.estado || 'Activo'
...
async function onFormSubmit({ valid }: any) {
  ...
  const segmentoExistente = segmentos.value.find(segmento =>
    ...
    Number(segmento.id) !== Number(props.id)
  )
  ...
  try {
    await $fetch(server.HOST + '/api/v1/segmentos-vehiculos/' + props.id, {
      method: 'PUT',
